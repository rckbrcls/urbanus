...
import type { LatLng, BoundingBox, ViewMode, NodeEditMode, MapNode, EnrichedFeatureCollection } from '../types';
...
export function MapProvider({
  ...
  const startProcessing = useCallback(async () => {
    ...
    try {
      ...
      dispatch({ type: 'SET_STREETS_DATA', payload: streetsResult.geojson as unknown as EnrichedFeatureCollection });
  ...
  const setStreetsData = useCallback((data: EnrichedFeatureCollection | null) => {
  ...
  const applyNodeChanges = useCallback((): EnrichedFeatureCollection | null => {
    const { streetsData, nodes } = stateRef.current;
    if (!streetsData) return null;

    return nodesService.applyNodesToStreets(streetsData, nodes) as unknown as EnrichedFeatureCollection;
  }, [nodesService]);
