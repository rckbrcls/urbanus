...
import { BoundingBox, ProcessingStages, ProcessingErrors } from '../types';
...
export function useDataProcessing(options?: UseDataProcessingOptions) {
  ...
  // Buscar ruas
  const fetchStreets = useCallback(async (bbox: BoundingBox): Promise<boolean> => {
  ...
  // Buscar topografia
  const fetchTopography = useCallback(async (bbox: BoundingBox): Promise<boolean> => {
  ...
  const processData = useCallback(async (bbox: BoundingBox) => {
