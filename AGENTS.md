...
1. Subdivisão de arestas longas (VERDE) — `core/graph/sanitization.py`
   2.5. Subdivisão de trechos íngremes (> 15%) — `core/graph/sanitization.py`
1. Remoção de nós redundantes (VERMELHO) — `core/graph/sanitization.py`
1. Resolução de clusters de curva — `core/graph/sanitization.py`
   4.5. Espaçamento mínimo PV (80m) — `core/graph/sanitization.py`
1. Detecção de máximos/mínimos (AMARELO/AZUL_ESCURO) — `core/elevation/extrema.py`
   5.5. Detecção de quebra de greide (> 3%) — `core/graph/sanitization.py`
1. Roteamento gravitacional RSPH — `core/routing/rsph.py`
1. Resolução de pontos baixos (elevatórias) — `core/optimizer/low_points.py`
1. Dimensionamento hidráulico NBR 9649 + invert elevation — `core/hydraulics/dimensioning.py`
1. Atribuição de acessórios (PV/TIL/TL/CP) — `core/graph/accessories.py`
